...
import (
	...
	"github.com/hantdev/sina/bootstrap/events/producer"
	"github.com/hantdev/sina/bootstrap/mocks"
...
func TestAdd(t *testing.T) {
	...
	for _, tc := range cases {
		tc.session = mitrasauthn.Session{UserID: validID, DomainID: tc.domainID, DomainUserID: validID}
		sdkCall := tv.sdk.On("Client", tc.config.ClientID, tc.domainID, tc.token).Return(sinasdk.Client{ID: tc.config.ClientID, Credentials: sinasdk.ClientCredentials{Secret: tc.config.ClientSecret}}, errors.NewSDKError(tc.clientErr))
...
func TestUpdateConnections(t *testing.T) {
	...
	for _, tc := range cases {
		tc.session = mitrasauthn.Session{UserID: validID, DomainID: tc.domainID, DomainUserID: validID}
		sdkCall := tv.sdk.On("Channel", mock.Anything, tc.domainID, tc.token).Return(sinasdk.Channel{}, tc.channelErr)
...
func TestUpdateCert(t *testing.T) {
	...
	lastID := "0"
	for _, tc := range cases {
		tc.session = mitrasauthn.Session{UserID: tc.userID, DomainID: tc.domainID, DomainUserID: validID}
...
func TestList(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			userID:   validID,
			domainID: domainID,
			session:  mitrasauthn.Session{UserID: validID, DomainID: domainID, DomainUserID: validID, SuperAdmin: true},
			config: bootstrap.ConfigsPage{
		...
		},
		{
			...
			userID:   validID,
			domainID: domainID,
			session:  mitrasauthn.Session{UserID: validID, DomainID: domainID, DomainUserID: validID},
			config: bootstrap.ConfigsPage{
				Total:   uint64(len(saved)),
				Offset:  0,
				Limit:   10,
