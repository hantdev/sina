...
import (
	...
	sinasdk "github.com/hantdev/mitras/pkg/sdk"
)
...
type Service interface {
	...
	Add(ctx context.Context, session mitrasauthn.Session, token string, cfg Config) (Config, error)

	// View returns Client Config with given ID belonging to the user identified by the given token.
	View(ctx context.Context, session mitrasauthn.Session, id string) (Config, error)

	// Update updates editable fields of the provided Config.
	Update(ctx context.Context, session mitrasauthn.Session, cfg Config) error

	// UpdateCert updates an existing Config certificate and token.
	// A non-nil error is returned to indicate operation failure.
	UpdateCert(ctx context.Context, session mitrasauthn.Session, clientID, clientCert, clientKey, caCert string) (Config, error)

	// UpdateConnections updates list of Channels related to given Config.
	UpdateConnections(ctx context.Context, session mitrasauthn.Session, token, id string, connections []string) error

	// List returns subset of Configs with given search params that belong to the
	// user identified by the given token.
	List(ctx context.Context, session mitrasauthn.Session, filter Filter, offset, limit uint64) (ConfigsPage, error)

	// Remove removes Config with specified token that belongs to the user identified by the given token.
	Remove(ctx context.Context, session mitrasauthn.Session, id string) error
	...
	ChangeState(ctx context.Context, session mitrasauthn.Session, token, id string, state State) error
...
func New(policyService policies.Service, configs ConfigRepository, sdk sinasdk.SDK, encKey []byte, idp mitras.IDProvider) Service {
...
func (bs bootstrapService) Add(ctx context.Context, session mitrasauthn.Session, token string, cfg Config) (Config, error) {
	...
	id := cfg.ClientID
	sinaClient, err := bs.client(session.DomainID, id, token)
	if err != nil {
		return Config{}, errors.Wrap(errClientNotFound, err)
	}

	for _, channel := range cfg.Channels {
		if channel.DomainID != sinaClient.DomainID {
			return Config{}, errors.Wrap(svcerr.ErrMalformedEntity, errNotInSameDomain)
		}
	}

	cfg.ClientID = sinaClient.ID
	cfg.DomainID = session.DomainID
	cfg.State = Inactive
	cfg.ClientSecret = sinaClient.Credentials.Secret
...
func (bs bootstrapService) View(ctx context.Context, session mitrasauthn.Session, id string) (Config, error) {
...
func (bs bootstrapService) Update(ctx context.Context, session mitrasauthn.Session, cfg Config) error {
	cfg.DomainID = session.DomainID
...
func (bs bootstrapService) UpdateCert(ctx context.Context, session mitrasauthn.Session, clientID, clientCert, clientKey, caCert string) (Config, error) {
...
func (bs bootstrapService) UpdateConnections(ctx context.Context, session mitrasauthn.Session, token, id string, connections []string) error {
	cfg, err := bs.configs.RetrieveByID(ctx, session.DomainID, id)
	if err != nil {
		return errors.Wrap(errUpdateConnections, err)
	}
...
func (bs bootstrapService) List(ctx context.Context, session mitrasauthn.Session, filter Filter, offset, limit uint64) (ConfigsPage, error) {
...
func (bs bootstrapService) Remove(ctx context.Context, session mitrasauthn.Session, id string) error {
...
func (bs bootstrapService) ChangeState(ctx context.Context, session mitrasauthn.Session, token, id string, state State) error {
	cfg, err := bs.configs.RetrieveByID(ctx, session.DomainID, id)
	if err != nil {
...
func (bs bootstrapService) client(domainID, id, token string) (sinasdk.Client, error) {
	// If Client ID is not provided, then create new client.
	if id == "" {
		id, err := bs.idProvider.ID()
		if err != nil {
			return sinasdk.Client{}, errors.Wrap(errCreateClient, err)
		}
		client, sdkErr := bs.sdk.CreateClient(sinasdk.Client{ID: id, Name: "Bootstrapped Client " + id}, domainID, token)
		if sdkErr != nil {
			return sinasdk.Client{}, errors.Wrap(errCreateClient, sdkErr)
		}
		return client, nil
	}
	...
	if sdkErr != nil {
		return sinasdk.Client{}, errors.Wrap(ErrClients, sdkErr)
	}
	return client, nil
}
