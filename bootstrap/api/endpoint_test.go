...
import (
	...
	bsapi "github.com/hantdev/sina/bootstrap/api"
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			if tc.token == validToken {
				tc.session = mitrasauthn.Session{DomainUserID: domainID + "_" + validID, UserID: validID, DomainID: domainID}
			}
...
func TestList(t *testing.T) {
	...
	for i := 0; i < changedStateNum; i++ {
		...
		err := svc.ChangeState(context.Background(), mitrasauthn.Session{}, validToken, list[i].ClientID, state)
