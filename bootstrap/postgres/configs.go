...
const cleanupQuery = `DELETE FROM channels ch WHERE NOT EXISTS (
						 SELECT channel_id FROM connections c WHERE ch.sina_channel = c.channel_id);`
...
func (cr configRepository) Save(ctx context.Context, cfg bootstrap.Config, chsConnIDs []string) (clientID string, err error) {
	q := `INSERT INTO configs (sina_client, domain_id, name, client_cert, client_key, ca_cert, sina_secret, external_id, external_key, content, state)
	VALUES (:sina_client, :domain_id, :name, :client_cert, :client_key, :ca_cert, :sina_secret, :external_id, :external_key, :content, :state)`
...
func (cr configRepository) RetrieveByID(ctx context.Context, domainID, id string) (bootstrap.Config, error) {
	q := `SELECT sina_client, sina_secret, external_id, external_key, name, content, state, client_cert, ca_cert
		  FROM configs
		  WHERE sina_client = :sina_client AND domain_id = :domain_id`
	...
	q = `SELECT sina_channel, name, metadata FROM channels ch
		 INNER JOIN connections conn
		 ON ch.sina_channel = conn.channel_id AND ch.domain_id = conn.domain_id
		 WHERE conn.config_id = :sina_client AND conn.domain_id = :domain_id`
...
func (cr configRepository) RetrieveAll(ctx context.Context, domainID string, clientIDs []string, filter bootstrap.Filter, offset, limit uint64) bootstrap.ConfigsPage {
	...
	q := `SELECT sina_client, sina_secret, external_id, external_key, name, content, state
		  FROM configs %s ORDER BY sina_client LIMIT $%d OFFSET $%d`
...
func (cr configRepository) RetrieveByExternalID(ctx context.Context, externalID string) (bootstrap.Config, error) {
	q := `SELECT sina_client, sina_secret, external_key, domain_id, name, client_cert, client_key, ca_cert, content, state
		  FROM configs
...
func (cr configRepository) Update(ctx context.Context, cfg bootstrap.Config) error {
	q := `UPDATE configs SET name = :name, content = :content WHERE sina_client = :sina_client AND domain_id = :domain_id `
...
func (cr configRepository) UpdateCert(ctx context.Context, domainID, clientID, clientCert, clientKey, caCert string) (bootstrap.Config, error) {
	q := `UPDATE configs SET client_cert = :client_cert, client_key = :client_key, ca_cert = :ca_cert WHERE sina_client = :sina_client AND domain_id = :domain_id 
	RETURNING sina_client, client_cert, client_key, ca_cert`
...
func (cr configRepository) Remove(ctx context.Context, domainID, id string) error {
	q := `DELETE FROM configs WHERE sina_client = :sina_client AND domain_id = :domain_id`
	dbcfg := dbConfig{
		ClientID: id,
		DomainID: domainID,
	}
...
func (cr configRepository) ChangeState(ctx context.Context, domainID, id string, state bootstrap.State) error {
	q := `UPDATE configs SET state = :state WHERE sina_client = :sina_client AND domain_id = :domain_id;`
...
func (cr configRepository) ListExisting(ctx context.Context, domainID string, ids []string) ([]bootstrap.Channel, error) {
	...
	q := "SELECT sina_channel, name, metadata FROM channels WHERE domain_id = $1 AND sina_channel = ANY ($2)"
...
func (cr configRepository) RemoveClient(ctx context.Context, id string) error {
	q := `DELETE FROM configs WHERE sina_client = $1`
...
func (cr configRepository) UpdateChannel(ctx context.Context, c bootstrap.Channel) error {
	...
	q := `UPDATE channels SET name = :name, metadata = :metadata, updated_at = :updated_at, updated_by = :updated_by 
			WHERE sina_channel = :sina_channel`
...
func (cr configRepository) RemoveChannel(ctx context.Context, id string) error {
	q := `DELETE FROM channels WHERE sina_channel = $1`
...
func buildRetrieveQueryParams(domainID string, clientIDs []string, filter bootstrap.Filter) (string, []interface{}) {
	...
	if len(clientIDs) != 0 {
		queries = append(queries, fmt.Sprintf("sina_client IN ('%s')", strings.Join(clientIDs, "','")))
	} else if domainID != "" {
...
func insertChannels(domainID string, channels []bootstrap.Channel, tx *sqlx.Tx) error {
	...
	q := `INSERT INTO channels (sina_channel, domain_id, name, metadata, parent_id, description, created_at, updated_at, updated_by, status)
		  VALUES (:sina_channel, :domain_id, :name, :metadata, :parent_id, :description, :created_at, :updated_at, :updated_by, :status)`
...
type dbConfig struct {
	...
	ClientID     string          `db:"sina_client"`
	ClientSecret string          `db:"sina_secret"`
