SINA_DOCKER_IMAGE_NAME_PREFIX ?= ghcr.io/hantdev/sina
BUILD_DIR = build
...
ifneq ($(SINA_MESSAGE_BROKER_TYPE),)
    SINA_MESSAGE_BROKER_TYPE := $(SINA_MESSAGE_BROKER_TYPE)
else
    SINA_MESSAGE_BROKER_TYPE=nats
endif

ifneq ($(SINA_ES_TYPE),)
    SINA_ES_TYPE := $(SINA_ES_TYPE)
else
    SINA_ES_TYPE=nats
endif
...
define compile_service
	...
	go build -tags $(SINA_MESSAGE_BROKER_TYPE) --tags $(SINA_ES_TYPE) -ldflags "-s -w \
	-X 'github.com/hantdev/sina.BuildTime=$(TIME)' \
	-X 'github.com/hantdev/sina.Version=$(VERSION)' \
	-X 'github.com/hantdev/sina.Commit=$(COMMIT)'" \
...
define make_docker
	...
	docker build \
		...
		--build-arg TIME=$(TIME) \
		--tag=$(SINA_DOCKER_IMAGE_NAME_PREFIX)/$(svc) \
		-f docker/Dockerfile .
endef
...
ifdef pv
	...
	docker volume ls -f name=$(SINA_DOCKER_IMAGE_NAME_PREFIX) -f dangling=true -q | xargs -r docker volume rm
endif
...
define docker_push
	for svc in $(SERVICES); do \
		docker push $(SINA_DOCKER_IMAGE_NAME_PREFIX)/$$svc:$(1); \
	done
endef
...
release:
	...
	$(MAKE) dockers
	for svc in $(SERVICES); do \
		docker tag $(SINA_DOCKER_IMAGE_NAME_PREFIX)/$$svc $(SINA_DOCKER_IMAGE_NAME_PREFIX)/$$svc:$(version); \
	done
...
run_addons: check_certs
	...
	@for SVC in $(RUN_ADDON_ARGS); do \
		SINA_ADDONS_CERTS_PATH_PREFIX="../."  docker compose -f docker/addons/$$SVC/docker-compose.yaml -p $(DOCKER_PROJECT) --env-file ./docker/.env $(DOCKER_COMPOSE_COMMAND) $(args) & \
	done