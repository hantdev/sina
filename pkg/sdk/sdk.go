...
type sinaSDK struct {
	...
	client         *http.Client
	curlFlag       bool
	msgContentType mitrasSDK.ContentType
...
func NewSDK(conf Config) SDK {
	mitrasSDK := mitrasSDK.NewSDK(mitrasSDK.Config{
...
func (sdk sinaSDK) processRequest(method, reqUrl, token string, data []byte, headers map[string]string, expectedRespCodes ...int) (http.Header, []byte, errors.SDKError) {
	...
	req.Header.Add("Content-Type", string(mitrasSDK.CTJSON))
	...
	if token != "" {
		if !strings.Contains(token, mitrasSDK.ClientPrefix) {
			token = mitrasSDK.BearerPrefix + token
		}
...
func (sdk sinaSDK) withQueryParams(baseURL, endpoint string, pm PageMetadata) (string, error) {
	q, err := pm.query()
	if err != nil {
		return "", err
	}
