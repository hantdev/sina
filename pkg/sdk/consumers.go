...
func (sdk sinaSDK) CreateSubscription(topic, contact, token string) (string, errors.SDKError) {
	sub := Subscription{
		Topic:   topic,
		Contact: contact,
	}
...
func (sdk sinaSDK) ListSubscriptions(pm PageMetadata, token string) (SubscriptionPage, errors.SDKError) {
...
func (sdk sinaSDK) ViewSubscription(id, token string) (Subscription, errors.SDKError) {
...
func (sdk sinaSDK) DeleteSubscription(id, token string) errors.SDKError {
	url := fmt.Sprintf("%s/%s/%s", sdk.usersURL, subscriptionEndpoint, id)
