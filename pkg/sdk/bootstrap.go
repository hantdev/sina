...
// SINAClient represents corresponding Mitras Client ID.
// SINAKey is key of corresponding Mitras Client.
// SINAChannels is a list of Mitras Channels corresponding Mitras Client connects to.
...
func (ts *BootstrapConfig) UnmarshalJSON(data []byte) error {
	...
	if channelData, ok := rawData["channels"]; ok {
		...
		if err := json.Unmarshal(channelData, &stringData); err == nil {
			...
			var channels []mitrasSDK.Channel
...
func (sdk sinaSDK) AddBootstrap(cfg BootstrapConfig, domainID, token string) (string, errors.SDKError) {
...
func (sdk sinaSDK) Bootstraps(pm PageMetadata, domainID, token string) (BootstrapPage, errors.SDKError) {
...
func (sdk sinaSDK) Whitelist(clientID string, state int, domainID, token string) errors.SDKError {
	if clientID == "" {
...
func (sdk sinaSDK) ViewBootstrap(id, domainID, token string) (BootstrapConfig, errors.SDKError) {
	if id == "" {
...
func (sdk sinaSDK) UpdateBootstrap(cfg BootstrapConfig, domainID, token string) errors.SDKError {
	if cfg.ClientID == "" {
		return errors.NewSDKError(apiutil.ErrMissingID)
	}
...
func (sdk sinaSDK) UpdateBootstrapCerts(id, clientCert, clientKey, ca, domainID, token string) (BootstrapConfig, errors.SDKError) {
	if id == "" {
		return BootstrapConfig{}, errors.NewSDKError(apiutil.ErrMissingID)
	}
...
func (sdk sinaSDK) UpdateBootstrapConnection(id string, channels []string, domainID, token string) errors.SDKError {
	if id == "" {
		return errors.NewSDKError(apiutil.ErrMissingID)
	}
...
func (sdk sinaSDK) RemoveBootstrap(id, domainID, token string) errors.SDKError {
	if id == "" {
		return errors.NewSDKError(apiutil.ErrMissingID)
	}
	url := fmt.Sprintf("%s/%s/%s/%s", sdk.bootstrapURL, domainID, configsEndpoint, id)
...
func (sdk sinaSDK) Bootstrap(externalID, externalKey string) (BootstrapConfig, errors.SDKError) {
	if externalID == "" {
		return BootstrapConfig{}, errors.NewSDKError(apiutil.ErrMissingID)
	}
	url := fmt.Sprintf("%s/%s/%s", sdk.bootstrapURL, bootstrapEndpoint, externalID)

	_, body, err := sdk.processRequest(http.MethodGet, url, mitrasSDK.ClientPrefix+externalKey, nil, nil, http.StatusOK)
	if err != nil {
		return BootstrapConfig{}, err
	}
...
func (sdk sinaSDK) BootstrapSecure(externalID, externalKey, cryptoKey string) (BootstrapConfig, errors.SDKError) {
	if externalID == "" {
		return BootstrapConfig{}, errors.NewSDKError(apiutil.ErrMissingID)
	}
	...
	_, body, sdkErr := sdk.processRequest(http.MethodGet, url, mitrasSDK.ClientPrefix+encExtKey, nil, nil, http.StatusOK)
	if sdkErr != nil {
		return BootstrapConfig{}, sdkErr
	}
