...
func (pr postgresRepo) saveJSON(ctx context.Context, msgs mitrasjson.Messages) error {
...
func (pr postgresRepo) insertJSON(ctx context.Context, msgs mitrasjson.Messages) error {
	tx, err := pr.db.BeginTxx(ctx, nil)
	if err != nil {
		return errors.Wrap(errSaveMessage, err)
	}
