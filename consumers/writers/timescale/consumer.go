...
func (tr timescaleRepo) saveJSON(ctx context.Context, msgs mitrasjson.Messages) error {
...
func (tr timescaleRepo) insertJSON(ctx context.Context, msgs mitrasjson.Messages) error {
	tx, err := tr.db.BeginTxx(ctx, nil)
	if err != nil {
		return errors.Wrap(errSaveMessage, err)
	}
...
func toJSONMessage(msg mitrasjson.Message) (jsonMessage, error) {
	data := []byte("{}")
	if msg.Payload != nil {
